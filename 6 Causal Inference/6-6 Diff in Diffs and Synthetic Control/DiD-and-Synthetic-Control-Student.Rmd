...
kansas <- 
  ...
  mutate(treatment = ifelse(state == "Kansas" & year_qtr >= 2012.5, 1, 0))
...
post_diff <-
  test %>% filter(year_qtr == 2012.75) %>%
  select(state, lngdpcapita) %>%
  pivot_wider(names_from = state,
              values_from = lngdpcapita) %>%
  summarise(Colorado - Kansas)
...
kansas %>%
  ...
  # process
  # ---------
  filter(state %in% c("Kansas","Colorado", "Iowa")) %>%
  # plotting all of the time periods -- not filtering out any of them
